...
namespace Rocketnote.Notes
{
	public class NotesAdder : IDisposable
	{
		...
		//
		public void AddNote(string title, string content, bool isHighPriority, ObservableCollection<Note> notesList)
		{
			/*
			 * CEL:
			 * Dodaje nową notatkę do notatnika
			...
			if (notebookCount <= MAX_NOTES_COUNT)
			{
				...
				notesList.Add(CreateNewNote(title, content, isHighPriority));
			}
		...
		// Tworzy nową notatkę
		//
        public Note CreateNewNote(string title, string note, bool isHighPriority = false)
		{
			/*
			 * CEL:
            ...
            // Priorytet
            newNote.IsHighPriority = isHighPriority;
		...
		//
		public bool AddingCheck(List<Note> notesList)
		{
			/*
			 * CEL:
			 * Sprawdza czy można dodać nową notatkę
